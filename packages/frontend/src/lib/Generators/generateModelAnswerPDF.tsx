...
function addExamInfo(doc: jsPDF, exam: Exam, yPosition: number): void {
    // Ministry header
    doc.setFont("times", undefined, 70);
    ...
    // Add course info
    yPosition += 12;
    doc.setFont("times", undefined, 12);
...
function addSection(doc: jsPDF, section: Section, yPosition: number): number {
    ...
    doc.setFont("times", undefined, 14);
...
function addTextBlock(doc: jsPDF, text: string, yPosition: number, heading: string): number {
    ...
    doc.setFont("times", undefined, 11);
...
function addGeneralQuestions(doc: jsPDF, questions: Question[], yPosition: number, heading?: string): number {
    ...
    questions.forEach((q, index) => {
        ...
        if(q.sentence) {
            doc.text(`Correct Answer: ${q.answer}`, marginLeft, yPosition);
            yPosition += 10;
        }
        ...
        q.options?.forEach((option:any, i:any) => {
            ...
            if(q.answer === option) {
                const circleX = marginLeft + 16; // X position of the circle
                const circleY = yPosition - 2; // Y position of the circle
                const circleRadius = 4; // Circle radius
                doc.setDrawColor(0); // Set the color to black for the circle outline
                doc.ellipse(circleX, circleY, circleRadius, circleRadius);
            }

            // Option text
            const optionLines = doc.splitTextToSize(optionText, marginRight - marginLeft);
            doc.text(optionLines, marginLeft + 10, yPosition); // Offset text to the right of the circle
...
function addTrueFalseQuestions(doc: jsPDF, questions: Question[], yPosition: number): number {
    ...
    questions.forEach((q, index) => {
        ...
        doc.setFont("times", undefined, 11);
        doc.text(`${index + 1}. ${q.statement} ( ${q.answer} )`, marginLeft, yPosition);
        yPosition += 13;
    });
...
function addVocabularyMatching(doc: jsPDF, vocabList: Question[], yPosition: number): number {
    ...
    vocabList.forEach((vocab, index) => {
        ...
        doc.setFont("times", undefined, 11);
        const definitionText = `${index + 1}. (${(vocab as any).word }) ${(vocab as any).definition }.`;
...
function addExercises(doc: jsPDF, exercises: Exercise[], yPosition: number): number {
    ...
    exercises.forEach((exercise, index) => {
        ...
        yPosition += 15;
        doc.text(`Correct Answer: ${exercise.answer}.`, marginLeft, yPosition);
        yPosition += 15;
    });
