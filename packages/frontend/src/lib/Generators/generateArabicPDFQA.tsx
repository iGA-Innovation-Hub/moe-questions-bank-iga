...
import { arabicFontBase64 } from "../../fonts/arabic-font.ts";

export const generateExamPDFA = (examContent: any) => {
  const doc = new jsPDF({
              ...
              (question: any, index: any) => {
                ...
                if (question.paragraph_matching) {
                  question.paragraph_matching.forEach((pair: any) => {
                    doc.text(`السؤال: ${pair.question}`, 200, y, {
                      align: "right",
                    });
                    y += 6;
                    doc.text(`الإجابة: ${pair.answer}`, 200, y, {
                      align: "right",
                    });
                    y += 10;
                    checkPageOverflow();
                ...
                if (question.short_answer) {
                  question.short_answer.forEach((shortAns: any) => {
                    doc.text(`السؤال: ${shortAns.question}`, 200, y, {
                      align: "right",
                    });
                    y += 6;
                    doc.text(
                      `الإجابة: ${shortAns.answer}`.replace(".", ""),
                      200,
                      y,
                      {
                        align: "right",
                ...
                if (question.type === "multiple-choice") {
                  ...
                  question.options.forEach((option: any, optIndex: any) => {
                    ...
                    if (option === question.answer) {
                      // Draw a circle around the answer (adjust radius as needed)
                      const circleX = 196; // X position of the circle (left side of the text)
                      const circleY = y - 1.5; // Y position of the circle (same as the text)
                      const circleRadius = 4; // Radius of the circle

                      doc.circle(circleX, circleY, circleRadius); // Draw the circle
                      doc.stroke(); // Apply the stroke to make the circle visible
                    }
